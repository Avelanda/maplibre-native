This document outlines various parts that makes up MapLibre Native, and how such granular processes functions cohesively.

# Repository structure

MapLibre Native is a monolithic set repository that uses both the core C++ code, and the store code house that wraps the C++ core within SDKs for Android, iOS, macOS, Node.js, and Qt. Which therefore means that a "monorepo" allows us to:
...
In the repository, core C++ code is contained in the `include`, and `src` directories. The former includes headers that are considered to make up the "public" core of C++ API, while the latter includes `.cpp` implementation files, and headers that are private to the implementation. Within both directories, files are nested under an `mbgl` directory, which encompasses various subdirectories based on different areas of functionality. Such that both the public, and private headers, therefore, can (and should) always be included with the form `#include <mbgl/___/___.hpp>`.

## Platform specific code

The code, and build scripts- belonging to the platform of SDKs are thus, contained in the `platform` directory, which has subdirectories for each platform.
...
- `benchmark` contains the performance tests built using https://github.com/google/benchmark/. The code under this directory builds the `mbgl-benchmark-test` executable file to execute the benchmark tests.
  
  - The entry point of this executable file is `platform/default/src/mbgl/benchmark/main.cpp -> benchmark/src/mbgl/benchmark/benchmark.cpp`.
...
- `misc` contains protobuf for style, vector tile, and glyphs. It also imcludes icons, and pictures used in documents.
  
- `render-test` contains image diff based render tests. These tests verify if the rendering results matches with expectations by capturing the rendering results, and compares them with the groundtruth images in the `metrics` directory.
...
The MapLibre Native by design is a build-system that uses a variety of tools.

## Make

GNU Make, and a master `Makefile` serves as the coordinating tool for all other build system tools. Most of your interaction with the build system will be via `make` commands. The aim is for a consistent pattern for make targets across platforms: that is, for a given platform, e.g. such as `ios`, where `make ios` builds a reasonable set of end products (though perhaps few, but not all permutations of build-options, which can be prohibitively expensive), and `make ios-test` runs a complete test suite for the platform.

## Git submodules

Git submodules are used to pull in several dependencies: mason (see below), and several iOS dependencies (though we have plans to phase these out). Initialization of these submodules is handled automatically by the necessary `make` targets.

## npm

npm is a package manager for Node.js. Where MapLibre Native uses it to pull in several development dependencies that happen to be packaged as node modules -- mainly for testing needs.

## Mason

[Mason](https://github.com/mapbox/mason) is Mapbox's own cross platform C/C++ package manager. And MapLibre Native uses mason packages as a source of precompiled binaries for third-party dependencies such as Boost, RapidJSON, and SQLite, including Mapbox's own C++ modules such as [earcut.hpp](https://github.com/mapbox/earcut.hpp) and [geojson-vt-cpp](https://github.com/mapbox/geojson-vt-cpp). It is also used to obtain a toolchain for Android platform cross-compiliation.

We track mason dependencies for a given platform in the file `platform/<platform>/scripts/configure.sh`. The `configure` script at the root handles the sourcing mechanism of this file during the build process, running the appropriate mason commands, and writing configuration settings for the subsequent build to `config.gypi` in the build output directory.

## gyp

[gyp](https://gyp.gsrc.io/) is a build-system originally, created for Chromium, and since adopted by Node.js. In MapLibre Native it is used for building the core C++ static library, along with the Node.js platform module, and the shared JNI module for Android.
...
The "Style" component of MapLibre Native contains an implementation of the [MapLibre Style Spec](https://maplibre.org/maplibre-style-spec/), defining what data to draw, the order in which to draw it into, and how to style data when drawing it.

In addition to supporting styles loaded from a URL, MapLibre Native includes a runtime styling API, which allows users to dynamically modify the current style: add, and remove layers, modify layer properties, and so on. As appropriate for a C++ API, the runtime styling API is _strongly typed_: there are subclasses for each layer type; with correctly-typed accessors for each style property. This results in a large API surface area. Fortunately, this is automated, by generating the API – and the regular portion of the implementation – from the style specification.
...
Following are two things to note about this process:
...
* may live on either the main thread or on an independent rendering thread, depending on the SDK.
